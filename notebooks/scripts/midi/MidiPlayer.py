...
from .MidiLoader import MidiLoader

class MidiPlayer:
    """
    Class for real-time playback of MIDI files using a MidiSynth instance.
    Stores message_dict, program_dict, pitch_df from MidiLoader and keeps
    track of playback time, currently playing channels.

    @methods:
    - load_midi(midi_file_path: str): Load a MIDI file from a file path
    ...
    def load_midi(self, midi_file_path: str):
        """
        Load a MIDI file from a file path (str) and parses + stores data
        as instance variables.
        @param:
            - midi_file_path: str, path to MIDI file
        @side_effects:
            - Sets message_dict, program_dict, pitch_df
    ...
    def _play(self, start_time=0):
        ...
        Converts message_dict into a list of all message times
        ...
        for _, program_change_msg in self.program_dict.items():
            self.MidiSynth.handle_midi(program_change_msg)
        
        all_message_times = list(self.message_dict.keys())
        ...
        for index in range(start_index, len(all_message_times)):
            ...
            current_messages = self.message_dict[current_time]
